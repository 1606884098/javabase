 ...
 *  *
 *  *     jni
 *  *     JNI是Java Native Interface的缩写，它提供了若干的API实现了Java和其他语言的通信桥梁（主要是C&C++）
 ...
 *  4.2连接验证
    ...
    4.4内存分配
        ...
        对于普通变量，一个线程中更新的值，不能马上反应在其他变量中
         ...
         保证可见性的方法
         volatile volatile规则：volatile变量的写，先发生于读  指令重排禁止
...
锁
 ...
 Monitor
 Monitor可以类比为一个特殊的房间，这个房间中有一些被保护的数据，Monitor保证每次
 只能有一个线程能进入这个房间进行访问被保护的数据，进入房间即为持有Monitor，退出
 房间即为释放Monitor。

 Java虚拟机给每个对象和class字节码都设置了一个监听器Monitor，用于检测并发代码的
 重入，同时在Object类中还提供了notify和wait方法来对线程进行控制。
 ...
 上述代码表示在进入同步代码块之前，先要去获取obj的Monitor，如果已经被其他线程获取了，那么当前线
 程必须等待直至其他线程释放obj的Monitor
 这里的obj可以是类.class，表示需要去获取该类的字节码的Monitor，获取后，其他线程无法再去获取到
 class字节码的Monitor了，即无法访问属于类的同步的静态方法了，但是对于对象的实例方法的访问不受影响
